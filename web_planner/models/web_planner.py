...
class Planner(models.Model):
    ...
    planner_type = fields.Selection([('help', 'Help'), ('config', 'Config')], string='Planner Type', required=True)
    ...
    def get_planner_for_menu_id(self, **kargs):
        return self.search([('menu_id', '=', kargs)])
    ...
    def prepare_backend_url(self, action_xml_id, view_type=None, module_name=None):
        ...
        if action:
            ...
            modes = [x if x != 'tree' else 'list' for x in
                     action.view_mode.split(',')]  # change 'tree' to 'list' for the JS code
